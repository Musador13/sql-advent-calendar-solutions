-- SQL Advent Calendar - Day 24
-- Title: New Year Goals - User Type Analysis
-- Difficulty: hard
--
-- Question:
-- As the New Year begins, the goals tracker team wants to understand how user types differ. How many completed goals does the average user have in each user_type?
--
-- As the New Year begins, the goals tracker team wants to understand how user types differ. How many completed goals does the average user have in each user_type?
--

-- Table Schema:
-- Table: user_goals
--   user_id: INT
--   user_type: VARCHAR
--   goal_id: INT
--   goal_status: VARCHAR
--

-- My Solution:

with cte as (
  select 
    -- *,
    user_id,
    user_type,
    count(1) filter(where goal_status = 'Completed') avg_completed_goals
    -- count(goal_id) filter(where goal_status = 'Completed') over(PARTITION by user_id, user_type)::NUMERIC as avg_completed_goals
  from user_goals
  GROUP by 1,2
)
select 
  user_type,
  avg(avg_completed_goals)
from cte 
GROUP by 1
